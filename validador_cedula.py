"Programa que valida el numero de cedula"
import math

def cedula_valida(cedula):
    """ Funcion que permite conocer si un numero
    de cedula es valido"""
    if len(cedula) != 10:
        return "No valida: debe ser de 10 digitos"
    else:
        if int(cedula[0:2]) > 24:
            return "No valida: Los primeros dos digitos " \
                   "no pertenecen a una provincia"
        if int(cedula[2]) > 6:
        ...
        digito_validador = int(math.ceil(sum(resultado) / 10.0)) \
                           * 10 - sum(resultado)
